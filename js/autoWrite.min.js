...
class autoWrite {
    constructor(output, words, speed, wait) {
        ...
        if (this.isDeleting) {
            this.outputText = this.wordsText.substring(0, this.outputText.length - 1);
        } else {
            this.outputText = this.wordsText.substring(0, this.outputText.length + 1);
        }
...
typingAction(autoWriteDOM, autoWriteWords, autoWriteSpeed, autoWriteWait, ","); function typingAction(first, second, third, fourth, makeArray) {
    first.forEach((item, index) => {
        second.push(item.innerHTML.split(makeArray)); third.push(item.classList[1]); fourth.push(item.classList[2] !== undefined ? item.classList[2] : 99999999);
